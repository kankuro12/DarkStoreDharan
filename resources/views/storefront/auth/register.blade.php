...
<div class="max-w-md mx-auto my-12 bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
    <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-500">Join DarkStore for ultra-fast 30-min deliveries</p>
    </div>
    ...
    <div class="relative flex items-center py-5">
        ...
        <span class="flex-shrink-0 mx-4 text-xs font-medium text-slate-400 uppercase tracking-wider">Or sign up with email</span>
        <div class="flex-grow border-t border-slate-200"></div>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required autofocus class="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 rounded-xl px-4 py-2.5 text-sm transition outline-none">
            @error('name')
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 rounded-xl px-4 py-2.5 text-sm transition outline-none">
            @error('email')
                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <button type="submit" class="w-full bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold text-sm py-3 px-4 rounded-xl transition shadow-md active:scale-95 cursor-pointer mt-2">
            Create Account
        </button>
    </form>

    <div class="mt-6 text-center text-xs text-slate-600">
        Already have an account? <a href="{{ route('login') }}" class="text-amber-600 hover:text-amber-700 font-bold">Log in</a>
    </div>
</div>
@endsection
