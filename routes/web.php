...
Route::middleware('guest')->group(function () {
    Route::get('/login', [\App\Http\Controllers\Storefront\AuthController::class, 'showLogin'])->name('login');
    Route::post('/login', [\App\Http\Controllers\Storefront\AuthController::class, 'login']);
    Route::get('/register', [\App\Http\Controllers\Storefront\AuthController::class, 'showRegister'])->name('register');
    Route::post('/register', [\App\Http\Controllers\Storefront\AuthController::class, 'register']);
    
    // Socialite
    Route::get('/auth/{provider}/redirect', [\App\Http\Controllers\Storefront\AuthController::class, 'redirectProvider'])->name('social.redirect');
    Route::get('/auth/{provider}/callback', [\App\Http\Controllers\Storefront\AuthController::class, 'handleProviderCallback'])->name('social.callback');
});
Route::post('/logout', [\App\Http\Controllers\Storefront\AuthController::class, 'logout'])->name('logout')->middleware('auth');
